...
def days(deck,past):
    # today/yesterday
    deck_past=deck[(deck['year']==past['year'])&(deck['month_num']==past['month_num'])&(deck['date']==past['date'])]
    tmpGrp=deck_past
    return [deck_past,tmpGrp]

def months(deck,past):
    # month
    deck_past=deck[(deck['year']==past['year'])&(deck['month_num']==past['month_num'])]
...
def init(flag,df,datetimeStamp):
    ...
    pastDate={
    'w':{i:(x-DateOffset(weeks=datebuffer)).strftime(j) for i,j in datetimeStamp.items()},
    'd':{i:(x-DateOffset(days=datebuffer)).strftime(j) for i,j in datetimeStamp.items()},
    'm':{i:(x-DateOffset(months=datebuffer)).strftime(j) for i,j in datetimeStamp.items()}
    }
...
def statusData(flag,df,datetimeStamp):
    ...
    # deck_past
    if(deck_past.size == 0):
        historyDate="{}/{}/{}, week: {}".format(past['date'],
            past['month_num'],past['year'],past['week'])
